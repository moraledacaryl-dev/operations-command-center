...
| operations-command-center | yes | main | clean before this pass | active local workspace and GitHub main |
...
- Current apps: Accounting and POS only
...
- Current certs: `hiddenoasis.app`, `pos.hiddenoasis.app`
...
- `accounting.hiddenoasis.app`, `operations.hiddenoasis.app`, and `staff.hiddenoasis.app` need nginx vhosts and SSL certificates.
...
1. Verify/publish Accounting on `accounting.hiddenoasis.app` while leaving root unchanged.
...
5. Switch `hiddenoasis.app` to the static launcher only after Accounting subdomain is proven.

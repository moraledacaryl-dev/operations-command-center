...
export default function MarketingAnnotationStudio() {
  ...
  useEffect(() => {
    setLoaded(false); setHistory([]); setFuture([]); setSaved(''); setTextOpen(false);
    if (canonicalMode) {
  ...
  fitCanvasRef.current = () => {
    ...
    const fitted = Math.min(availableWidth / base.width, availableHeight / base.height);
    setZoom(Math.max(.1, Math.min(4, fitted)));
  };
  ...
  function loadBlob(blob: Blob, filename: string) {
    ...
    image.onload = () => {
      const base = baseCanvasRef.current; const annotation = annotationCanvasRef.current;
      ...
      const baseCtx = base.getContext('2d'); const annotationCtx = annotation.getContext('2d'); if (!baseCtx || !annotationCtx) return;
      baseCtx.clearRect(0, 0, width, height); baseCtx.drawImage(image, 0, 0, width, height); annotationCtx.clearRect(0, 0, width, height);
      URL.revokeObjectURL(url); setLoaded(true); setSourceName(filename); setHistory([]); setFuture([]); setSaved(''); setError(''); setTextOpen(false);
  ...
  function overlayUpload(event: ChangeEvent<HTMLInputElement>) {
    ...
    const mimeType = imageMime(file.name, file.type);
    if (!mimeType) { setError('Overlay must be a PNG, JPEG, or WebP image.'); return; }
    ...
    image.onload = () => {
      ...
      if (ctx) { ctx.globalCompositeOperation = 'source-over'; ctx.globalAlpha = 1; ctx.drawImage(image, x, y, width, height); }
      URL.revokeObjectURL(url); setSaved(''); setError('');
    };
    image.onerror = () => { URL.revokeObjectURL(url); setError('This overlay image could not be opened.'); };
    image.src = url;
  }
  ...
  function pointerMove(event: ReactPointerEvent<HTMLCanvasElement>) {
    ...
    const p = point(event); const scale = event.currentTarget.width / Math.max(1, event.currentTarget.getBoundingClientRect().width);
    ctx.lineCap = 'round'; ctx.lineJoin = 'round';
    if (tool === 'eraser') { ctx.globalCompositeOperation = 'destination-out'; ctx.globalAlpha = 1; ctx.strokeStyle = '#000'; ctx.lineWidth = size * 2 * scale; }
    else { ctx.globalCompositeOperation = 'source-over'; ctx.strokeStyle = color; ctx.globalAlpha = tool === 'highlighter' ? .28 : 1; ctx.lineWidth = (tool === 'highlighter' ? size * 3 : size) * scale; }
    ctx.lineTo(p.x, p.y); ctx.stroke(); ctx.globalAlpha = 1; ctx.globalCompositeOperation = 'source-over';
  }
  ...
  function addText() {
    const canvas = annotationCanvasRef.current; const value = textValue.trim();
    if (!canvas || !value) { setTextOpen(false); return; }
    checkpoint(); const ctx = canvas.getContext('2d'); if (!ctx) return;
    ctx.globalCompositeOperation = 'source-over'; ctx.globalAlpha = 1; ctx.fillStyle = color; ctx.font = `700 ${textSize}px system-ui, -apple-system, sans-serif`; ctx.textBaseline = 'top'; ctx.fillText(value, textPoint.x, textPoint.y);
    setTextOpen(false); setTextValue(''); setSaved('');
  }
  ...
  return <main className={styles.page}>
    <header className={styles.hero}><div className={styles.heroCopy}><div className={styles.eyebrow}>Marketing · Creative review</div><h1>Annotation studio</h1><p>Mark up the selected creative without changing the original. Draw, highlight, add text directly on the image, or place an image overlay, then save the result as a new version.</p></div><Link className={styles.back} href="/posts">← Back to Marketing</Link></header>
    {error ? <div className="pill urgent" role="alert">{error}</div> : null}
    <div className={styles.shell}>
      <aside className={styles.rail} aria-label="Annotation controls">
        <section className={styles.section}><div className={styles.sectionTitle}>Creative source</div>{canonicalMode ? <div className={styles.saveState}>{canonicalConcept?.title || `Content concept #${conceptId}`}</div> : <select aria-label="Marketing post" className={styles.select} value={postId} onChange={e => setPostId(e.target.value)}><option value="">Choose marketing post</option>{posts.map(post => <option key={post.id} value={post.id}>{post.title}</option>)}</select>}<select aria-label="Creative image version" className={styles.select} value={versionId} onChange={e => setVersionId(e.target.value)} disabled={(!canonicalMode && !postId) || !imageVersions.length}><option value="">{!imageVersions.length ? 'No image versions yet' : 'Choose image version'}</option>{imageVersions.map(version => <option key={version.id} value={version.id}>v{version.version_no || version.id} · {version.filename || 'Image asset'}</option>)}</select><button className={styles.secondary} onClick={loadSelectedVersion} disabled={!versionId || busy}>{busy ? 'Opening…' : 'Open version'}</button><label className={`${styles.upload} ${!loaded ? styles.uploadDisabled : ''}`}>Add image overlay<input aria-label="Overlay image" type="file" accept="image/png,image/jpeg,image/webp" onChange={overlayUpload} disabled={!loaded} /></label><span className={styles.saveState}>Overlay uploads are placed on top of the selected creative; they never replace the base image.</span></section>
        <section className={styles.section}><div className={styles.sectionTitle}>Tools</div><div className={styles.toolGrid}>{(['pen','highlighter','eraser','text'] as Tool[]).map(value => <button key={value} className={`${styles.tool} ${tool === value ? styles.toolActive : ''}`} onClick={() => { setTool(value); if (value !== 'text') setTextOpen(false); }} aria-pressed={tool === value}>{value === 'pen' ? '✎ Pen' : value === 'highlighter' ? '▰ Highlight' : value === 'eraser' ? '⌫ Eraser' : 'T Text'}</button>)}</div><span className={styles.saveState}>Shortcuts: P · H · E · T</span></section>
        ...
        <section className={styles.section}><div className={styles.sectionTitle}>Text size · {textSize}px</div><input aria-label="Text font size" className={styles.range} type="range" min="12" max="160" step="2" value={textSize} onChange={e => setTextSize(Number(e.target.value))} /><span className={styles.saveState}>Choose Text, then click the exact point on the creative where typing should begin.</span></section>
      ...
      <section className={styles.workspace}>
        ...
        <div ref={stageWrapRef} className={styles.stageWrap} tabIndex={0} role="region" aria-label="Creative annotation canvas"><div className={styles.canvasFrame} style={{ transform: `scale(${zoom})`, visibility: loaded ? 'visible' : 'hidden', position: loaded ? 'relative' : 'absolute' }}><canvas ref={baseCanvasRef} className={styles.baseCanvas} /><canvas ref={annotationCanvasRef} className={styles.canvas} onClick={placeText} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} />{textOpen ? <input autoFocus aria-label="Text annotation" className={styles.inlineText} value={textValue} onChange={e => setTextValue(e.target.value)} onPointerDown={e => e.stopPropagation()} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addText(); } if (e.key === 'Escape') { e.preventDefault(); setTextOpen(false); setTextValue(''); } }} style={{ left: textPoint.x, top: textPoint.y, fontSize: textSize, color }} placeholder="Type…" /> : null}</div>{!loaded ? <div className={styles.empty}><strong>Open a creative to start annotating</strong><span>Select an existing image version first. The studio will automatically fit it to the available workspace.</span></div> : null}</div>

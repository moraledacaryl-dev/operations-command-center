...
export default function ReviewPage() {
  ...
  async function decide(action: string) {
    ...
    setError('');
    try {
      if (kind === 'Approvals') await api.decideApproval(selected.id, action, { note: note.trim() });
      ...
      else await api.status(resourceFor(kind), selected.id, action, note.trim() || undefined);
      setSelected(null);
      await load();
    } catch (err: any) {
  ...
  return <>
    <Top eyebrow="Decision queue" title="Review" right={<button className="btn secondary" onClick={load} disabled={loading || submitting}>{loading ? 'Refreshing…' : 'Refresh'}</button>} />
    ...
    <section className="panel">
      ...
      {loading ? <div className="empty">Loading review queue…</div> : items.length ? <div className="grid">
        {items.map(item => <button type="button" className="card" key={`${item.review_kind}-${item.id}`} onClick={() => open(item)} style={{ textAlign: 'left', width: '100%' }}>
      ...
      <div className="panel">
        ...
        <div className="form" style={{ marginTop: 12 }}>
          ...
          <div className="toolbar" style={{ marginBottom: 0 }}>
            ...
            {kind === 'Posts' && <><button className="btn small" onClick={() => decide('OK')} disabled={submitting}>Approve creative</button><button className="btn small secondary" onClick={() => decide('Fix')} disabled={submitting}>Needs fix</button></>}
          </div>
        </div>
      </div>
    </Drawer>

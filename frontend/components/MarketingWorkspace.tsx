...
function dayLabel(value: Date) {
  return new Intl.DateTimeFormat('en-PH', { weekday: 'short', day: 'numeric' }).format(value);
}

function dayAccessibleLabel(value: Date) {
...
function range(anchor: Date, view: string) {
  ...
  if (view === 'Month') {
    ...
    to.setTime(from.getTime()); to.setDate(to.getDate() + 7);
  } else {
    from.setDate(from.getDate() - 14); from.setHours(0, 0, 0, 0);
    to.setDate(to.getDate() + 45); to.setHours(0, 0, 0, 0);
  }
  return { from, to };
}
...
export function MarketingWorkspace({ departmentId, canManage }: { departmentId: number; canManage: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const [campaignRows, conceptRows, calendarRows] = await Promise.all([
        api.marketingCampaigns(),
        api.marketingConcepts(),
        api.marketingCalendar({ date_from: dates.from.toISOString(), date_to: dates.to.toISOString(), department_id: departmentId }),
      ]);
      setCampaigns(campaignRows.filter(row => Number(row.department_id) === departmentId));
      setConcepts(conceptRows.filter(row => Number(row.campaign?.department_id) === departmentId));
      setCalendar(calendarRows);
    } catch (err: any) {
      setError(err.message || 'The campaign calendar could not be loaded.');
    }
  ...
  function openConceptForm(scheduledAt?: string) {
    if (!canManage || !campaigns.length) return;
    setShowCampaign(false);
    setConcept(value => ({
      ...value,
      campaign_id: value.campaign_id || String(campaigns[0].id),
      scheduled_at: scheduledAt ?? value.scheduled_at,
    }));
    setShowConcept(true);
    window.setTimeout(() => {
  ...
  async function createCampaign() {
    ...
    try {
      ...
      setCampaign({ name: '', objective: '', start_at: '', end_at: '' });
      setConcept(value => ({ ...value, campaign_id: String(created.id) }));
      setShowCampaign(false); setShowConcept(true); await load();
  ...
  async function transition(deliverable: Entity, action: string) {
    setBusy(true);
    try {
      await api.marketingDeliverableAction(deliverable.id, action, { note: note.trim() || null, final_url: finalUrl.trim() || null });
      setNote(''); setFinalUrl(''); await load();
      const refreshed = (await api.marketingConcepts()).find(row => row.id === selected?.id);
      if (refreshed) setSelected(refreshed);
    } catch (err: any) { setError(err.message || 'Platform workflow could not be updated.'); } finally { setBusy(false); }
  }

  async function reschedule(deliverable: Entity, value: string) {
    setBusy(true);
    try { await api.rescheduleMarketingDeliverable(deliverable.id, value ? new Date(`${value}T09:00:00+08:00`).toISOString() : null); await load(); } catch (err: any) { setError(err.message || 'Schedule could not be updated.'); } finally { setBusy(false); }
  }
